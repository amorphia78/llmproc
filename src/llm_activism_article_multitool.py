...
def load_config(config_file_path: str):
    try:
        ...
        return do_screening, do_coding, do_summarising, process_only_selected, stop_after, count_type, article_order_random_seed, output_article_full, output_article_summarised, output_article_summary_process, output_only_articles_passing_screening, output_detailed_word_counts, article_selection, article_exclusion_list, output_picture_tags
    except FileNotFoundError:
...
def llm_test():
    message = client.messages.create(
        ...
        max_tokens=1000,
        temperature=0,
        system="You are a news media article summariser focused on maintaining original article balance.",
        messages=[
            {
                "role": "user",
                "content": [
...
def summarise_article(article):
    ...
    prompt = prompt_summarise_intro + content + prompt_summarise_end
    response = llm_prompt(prompt, "summariser", article["title"])
...
def resummarise_article(article):
    ...
    prompt = ( prompt_resummarise_intro + article["text"] + prompt_resummarise_link1 + article["summary"] + prompt_resummarise_link2 +
               comparison_with_full_questions + prompt_resummarise_end + word_count_prompt )
    return llm_prompt( prompt, "summariser" )
...
def owe_focussed(article):
    ...
    prompt = prompt_owe_intro + title_and_body + prompt_owe_end
    yes_or_no = llm_prompt( prompt, "processor" )
...
def parse_and_validate_screening_response(response):
    ...
    if len(headers) != len(values) or len(headers) != len(screening_code_names):
        raise ValueError(f"Number of fields in response doesn't match screening_code_names {len(headers)} {len(values)} {len(headers)}{len(screening_code_names)}")
    if set(headers) != set(screening_code_names):
        raise ValueError("Headers in response don't match screening_code_names keys")
...
def prompt_llm_with_categorisation_message_with_prefill(prompt, prefill):
    return llm_prompt( prompt, "processor", prefill )

def full_screening(article):
    title_and_body = get_title_and_subtitle_and_article(article, article["text"])
    prompt = prompt_full_screening_intro + title_and_body + prompt_full_screening_end
    prefill = "\t".join(screening_code_names)
    response = llm_prompt( prompt, "processor", prefill )
...
def code_article(article, version):
    ...
    prompt = prompt_coding_intro + coding_form + prompt_coding_link + title_and_body + prompt_coding_end
    ...
    if code_names != rating_code_names:
...
def read_all_article_content():
    print("Reading input")
    base_path = "article_contents"
    all_json_data = {}
...
def output_coding_headers(file_name, do_screening, do_coding):
    ...
    screening_headers = ([*screening_code_names, "OWE_FOCUSSED", "PASSES_SCREENING"] if do_screening else [])
    coding_headers = (rating_code_names if do_coding else [])
...
def output_codes( file_name, article, do_coding, do_screening, do_summarising ):
    ...
    if do_screening:
        values += [str(article["screening_codes"][code_name]) for code_name in screening_code_names] + [
    ...
    with open(file_name, 'a') as coding_output_file:
        ...
        if do_coding and article["passes_screening"] == "Yes":
            coding_output_file.write(
                "\t" + "\t".join(str(article["codes_text"][code_name]) for code_name in rating_code_names) + "\n")
            if do_summarising and article["summarised"]:
                ...
                coding_output_file.write("\t" + "\t".join(
                    str(article["codes_summary"][code_name]) for code_name in rating_code_names) + "\n")
                coding_output_file.write("\t".join([article["id"], article["title"], article["url"],
                                                    "resummarised", str(article["resummary_word_count"])]) + "\t" * 11)
                coding_output_file.write("\t" + "\t".join(
                    str(article["codes_resummary"][code_name]) for code_name in rating_code_names) + "\n")
        else:
            coding_output_file.write("\n")
...
def main():
    articles = read_all_article_content()
    ...
    do_screening, do_coding, do_summarising, process_only_selected, stop_after, count_type, article_order_random_seed, output_article_full, output_article_summarised, output_article_summary_process, output_only_articles_passing_screening, output_detailed_word_counts, article_selection, article_exclusion_list, output_picture_tags = load_config("config.json")
    ...
    coding_output_filename = f"output_folders/coding_output/coding_output_{timestamp}.tsv"
